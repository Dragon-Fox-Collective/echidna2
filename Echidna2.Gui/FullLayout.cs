	...
	public override void OnPreNotify(UpdateNotification notification)
	{
		...
		if (laidOutChildren.Count != 0)
			RectTransform.MinimumSize = (
				Math.Max(RectTransform.MinimumSize.X, laidOutChildren.Max(child => child.MinimumSizeToParent.X) + LeftMargin + RightMargin),
				Math.Max(RectTransform.MinimumSize.Y, laidOutChildren.Max(child => child.MinimumSizeToParent.Y) + BottomMargin + TopMargin));
		else
			RectTransform.MinimumSize = (
		...
		foreach (RectTransform child in laidOutChildren)
		{
			...
			child.LocalSize = (interiorSize.X / child.LocalScale.X, interiorSize.Y / child.LocalScale.Y);
		}
	}
}